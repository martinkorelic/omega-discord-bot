...
module.exports = {
    name: 'punish',
    description: 'Punishes a member. Punished role must be set. Use \`setrole punished\` to set a punished role.',
    type: 'mod',
    args: true,
    guildOnly: true,
    usage: '<member> <reason>',
    ...
    execute(msg, args) {
        ...
        if (memberId == OMEGA_ID) {
            return msg.channel.send(handleError('Cannot punish myself!', 'user-mistake'));
        }
        ...
        if (!checkRoles(omegaRoles, requestedMember)) {
            return msg.channel.send(handleError('I am not allowed to punish members equal or above me. 😳', 'internal-error'));
        }

        if (!checkRoles(memberRoles, requestedMember) && !msg.member.hasPermission('ADMINISTRATOR')) {
            return msg.channel.send(handleError('You are not allowed to punish roles above you except if you are an administrator!','user-mistake'));
        }
        ...
        StateManager.serverAvailable(msg.guild.id, (oGuild) => {
            ...
            return oGuild.sendBotLog(msg, handleInfo(oGuild.punishMember(msg, memberId, reason), 'info'));
        });
